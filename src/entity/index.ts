...
function addReducerToNgModule(options: EntityOptions): Rule {
	return (host: Tree) => {
		if (!options.module) {
			return host;
		}
...
function addEffectToNgModule(options: EntityOptions): Rule {
	return (host: Tree) => {
		if (!options.module) {
			return host;
		}
...
function buildPath(options: EntityOptions) {
	...
	if (options.layer == modulePath) {
		path = normalize(`${options.path}/${modulePath}/entities/`);
	} else {
		path = normalize(`${options.path}/${options.layer}/${modulePath}/entities/`);
	}
...
export default function (options: EntityOptions): Rule {
	return async (host: Tree) => {
		...
		options.type = options.type ? `.${options.type}` : 'Entity';

		options.path = buildDefaultPath(project); // src/app/
		options.module = findModuleFromOptions(host, options);

		// Remap path
		options.path = buildPath(options); // src/app/{layer}/{modules}/entitys/
