...
export function addSymbolToNgModuleMetadata(
		...
		if (expr.properties.length == 0) {
			...
			if (matches) {
				toInsert =
					...
					`${indentBy(matches[2].length + 2)`${symbolName}`}${matches[0]}]`;
			} else {
	...
	if (node.kind == ts.SyntaxKind.ArrayLiteralExpression) {
		...
		position--;
		toInsert = `\n${indentBy(2)`${symbolName}`}\n	`;
	} else {
		// Get the indentation of the last element, if any.
		const text = node.getFullText(source);
		const matches = text.match(/^(\r?\n)(\s*)/);
		if (matches) {
			toInsert = `,${matches[1]}${indentBy(matches[2].length)`${symbolName}`}`;
		} else {
			toInsert = `, ${symbolName}`;
		}
	}
 ...
 */
export function addFeatureRouteForLayout(
	source: ts.SourceFile,
	fileToAdd: string,
 ...
 */
export function getStoreModuleDeclaration(source: ts.SourceFile): ts.Expression | undefined {
	const result = getDecoratorMetadata(source, 'NgModule', '@angular/core');
	const node = result[0] as ts.ObjectLiteralExpression;
	const matchingProperties = getMetadataField(node, 'imports');
	...
	return arrLiteral.elements
		.filter((el) => el.kind === ts.SyntaxKind.CallExpression)
		.find((el) => (el as ts.Identifier).getText().startsWith('StoreModule'));
}
...
export function addStateDeclarationToModule(
	...
	if (!routerModuleExpr) {
		throw new Error(`Couldn't find a store module declaration in ${fileToAdd}.`);
	}
	...
	let statesObj: ts.ObjectLiteralExpression | undefined;

	if (ts.isObjectLiteralExpression(statesArg)) {
		statesObj = statesArg;

		const text = statesObj.getFullText(source);
