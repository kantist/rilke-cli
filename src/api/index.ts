...
import { Schema as ApiOptions } from './schema';
...
function addProviderToNgModule(options: ApiOptions): Rule {
	return (host: Tree) => {
		...
		const apiPath =
			...
			'.api';
		const relativePath = buildRelativePath(modulePath, apiPath);
		const classifiedName = strings.classify(`${options.name}Api`);
...
function buildPath(options: ApiOptions) {
	const modulePath = normalize(dasherize(`${options.moduleName}`));

	let path;
	if (options.layer == modulePath) {
		path = normalize(`${options.path}/${modulePath}/api/`);
	} else {
		path = normalize(`${options.path}/${options.layer}/${modulePath}/api/`);
	}

	return path;
}

export default function (options: ApiOptions): Rule {
	return async (host: Tree) => {
		...
		// Remap path
		options.path = buildPath(options); // src/app/{layer}/{modules}/api/
