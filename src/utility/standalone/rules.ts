...
function getRootInsertionRule(
	...
	return async (host) => {
		...
		if (isStandaloneApp(host, mainFilePath)) {
			return (tree) =>
				addProviderToStandaloneBootstrap(tree, callback(codeBlock), mainFilePath, standaloneWrapperFunction);
		}
		...
		() => {
			...
			if (wrapperFunction) {
				...
				const result = CodeBlock.transformPendingCode(
					block.code`${block.external(wrapperFunction.name, wrapperFunction.module)}(${code.expression})`,
					fileToEdit
				);
...
function insertStandaloneRootProvider(tree: Tree, mainFilePath: string, expression: string): void {
	...
	if (bootstrapCall.arguments.length === 0) {
		throw new SchematicsException(
			`Cannot add provider to invalid bootstrapApplication call in ${bootstrapCall.getSourceFile().fileName}`
		);
	}
	...
	applyChangesToFile(tree, mainFilePath, [
		insertAfterLastOccurrence(targetCall.arguments, newAppConfig, mainFilePath, targetCall.getEnd() - 1),
	]);
}
 ...
 * @param expression Code expression to be inserted.
 */
function addProvidersExpressionToAppConfig(tree: Tree, appConfig: ResolvedAppConfig, expression: string): void {
