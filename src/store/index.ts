...
function addConfigurationToNgModule(options: StoreOptions): Rule {
	return (host: Tree) => {
		if (!options.module) {
			return host;
		}
...
function buildPath(options: StoreOptions) {
	...
	if (options.layer == modulePath) {
		path = normalize(`${options.path}/${modulePath}/stores/`);
	} else {
		path = normalize(`${options.path}/${options.layer}/${modulePath}/stores/`);
	}
